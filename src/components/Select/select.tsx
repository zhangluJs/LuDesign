...
export const Select:FC<SelectProps> = (props) => {
    ...
    const containerClass = classNames('lu-select', {
    ...
    <div className={containerClass} ref={containerRef}>
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="lu-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="lu-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="lu-tag" key={`tag-${index}`}>
                  {value}
